...
function createUser ($info){
    ...
    if(verifyUser($email, $username, null)){
...
function getUsers(){
    ...
    $surveyTaker = $mysqli->query($instruction);
    while($userResult = $surveyTaker->fetch_assoc()){
        $instruction = "SELECT id, email, username FROM Login WHERE id=$userResult[login_id]";
        $logInfo = $mysqli->query($instruction);
        $logInfoResult = $logInfo->fetch_assoc();
        ...
        $userToAdd=[
            "username"=>$logInfoResult[username],
            "email" => $logInfoResult[email],
            "name"=> $userResult[nombre],
...
function updateUser($info){
    ...
    $instruction = "SELECT password, email, username FROM Login WHERE id='$id'";
    $loginInfo = $mysqli->query($instruction);
    $loginInfoResult = $loginInfo->fetch_assoc();
    ...
    if($email != $loginInfoResult[email] || $username !=$loginInfoResult[username] ){
        $needToVerify = true;
        $readyToAdd = false;
        if($email != $loginInfoResult[email]  ){
            $emailToSearch = $email;
        }
        if($username != $loginInfoResult[username]  ){
            $usernameToSearch = $username;
        }
    }

    if($needToVerify){
        if(verifyUser($emailToSearch, $usernameToSearch, $id)){
            $readyToAdd = true;
        }
        else{
            $toReturn = [
...
function addPoints1 ($points, $type, $id)
{
    ...
    if ($mysqli->query($instruction) === TRUE) {
        if($type != 5){
            $instruction = "UPDATE Usuario SET puntos=puntos+$points WHERE login_id='$id'";
            if ($mysqli->query($instruction) === TRUE) {
                $toReturn = [
                    "status" => "win"
                ];
...
function addPoints ($info)
{
    ...
    if ($mysqli->query($instruction) === TRUE) {
        if($type != 5){
            $instruction = "UPDATE Usuario SET puntos=puntos+$pointsToAdd WHERE login_id='$id'";
            if ($mysqli->query($instruction) === TRUE) {
                $toReturn = [
                    "status" => "win"
                ];
...
function verifyUser ($email, $user, $id){
    global $mysqli;
    $email = htmlentities($email, ENT_QUOTES);
    $user = htmlentities($user, ENT_QUOTES);

    if($email != "" && $email != null && $user != "" && $user != null ){
        $instruction = "SELECT id FROM Login WHERE email='$email' OR username='$user' OR email='$user' OR username='$email'  AND NOT id=$id";

    }
    else if($email != "" || $email != null){
        $instruction = "SELECT id FROM Login WHERE (email='$email' OR username='$email') AND NOT id=$id";

    }
    else if($user != "" || $user != null){
        $instruction = "SELECT id FROM Login WHERE username='$user' OR email='$user' AND NOT id=$id";
    }
...
function updateAdmin($info){
    ...
    if($needToVerify){
        if(verifyUser($email, $username, $id)){
            $readyToAdd = true;
        }
    }
    if($readyToAdd){
...
function getTradedProducts(){
    ...
    $instruction = "SELECT id, status, producto_id, usuario_id, detalles, fecha FROM CanjeaProducto ";
...
//        $fecha = date_create_from_format('j-M-Y', $tradeProductResult[fecha]);
//        $date = date_format($fecha, 'Y-m-d');

        //$date = date( "Y-M-d", strtotime( $tradeProductResult[fecha]));
        $date=strftime("%Y-%b-%e", strtotime($tradeProductResult[fecha]));
        $productToAdd=[
...
function setPointsFromFile ($info){
    global $mysqli;
    $fileName=htmlentities($info->fileName, ENT_QUOTES);
    $monthToAdd = htmlentities($info->date, ENT_QUOTES);
    ...
    $status = "fail";
    if($monthToAdd!=0)
        $date = date("y-m-d", strtotime('2017-'.$monthToAdd.'-20'));
    else
        $date = date("y-m-d");
    ...
    while($keepGoing){
        ...
        if($id != null  && $id != "") {
            $distributionPoints = $objPHPExcel->getActiveSheet()->getCell("Q" . $i)->getValue();
            $selloutPoints = $objPHPExcel->getActiveSheet()->getCell("R" . $i)->getValue();
            $possiblePoints = $objPHPExcel->getActiveSheet()->getCell("S" . $i)->getValue();
            ...
            if ($mysqli->query($instruction) === TRUE) {
                ...
                if ($mysqli->query($instruction) === TRUE) {
                    ...
                    if ($mysqli->query($instruction) === TRUE) {
                        $instruction = "UPDATE Usuario SET puntos=puntos+$distributionPoints+$selloutPoints WHERE login_id='$id'";
                        if ($mysqli->query($instruction) === TRUE) {
                            $status = "win";
                        }
                        else{
...
//    echo "enviando corrao a ". $to. " con el nombre ".$name ."//";
...
';
    mail($to,"Bienvenido al Colgate Club", $html, $headers);
}
