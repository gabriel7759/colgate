...
function createUser ($info){
    ...
    if ($mysqli->query($instruction) === TRUE) {
        $lastId = $mysqli->insert_id;
        $instruction = "INSERT INTO Usuario (nombre, puntos, tienda_id, login_id, rfc, ruta, telefono, calle, colonia, numero, municipio, estado, cp) VALUES  ('$name', '$points', '$store', '$lastId', '$rfc', '$route', '$phone', '$street', '$suburb', '$number', '$city', '$state', '$cp')";
        if ($mysqli->query($instruction) === TRUE) {
            $toReturn = [
                "status" => "win",
            ];
